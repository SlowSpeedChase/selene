...
| [distill-essences](../src/workflows/distill-essences.ts) | every 5 min | processed_notes, raw_notes | processed_notes | Backfill/retry LLM essences for processed notes that still lack one |
...
| [folio-feedback](../src/workflows/folio-feedback.ts) | every 5 min | raw_notes, processed_notes | Folio project feedback files, raw_notes (status_folio) | Write Kindle-Scribe annotations back as markdown feedback files in each Folio project repo |
...
| [ingest](../src/workflows/ingest.ts) | webhook (POST /webhook/api/drafts); also called directly by eink-ingest & voice-ingest | — | raw_notes | Capture a note — dedupe by content hash, store it, link a calendar event |
