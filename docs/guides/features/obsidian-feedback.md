...
- **Honest ✓** — `applied_at` is stamped only when the LLM extraction parsed (`src/workflows/process-llm.ts`). If Ollama returned garbage and the note degraded to defaults, your feedback stays visibly pending (plain text, no ✓) — but it is **not** auto-retried: the note is still marked processed (long-standing degraded-extraction fallback), and the dedupe index skips the identical text on every future scan. It gets another chance only when you edit or extend the text (new text = new ingest), when you add another round of feedback, or when an operator rebuild re-derives the corpus (your intent history always rides along). Practical rule: if it stays plain with no ✓ after a couple of hours, edit the text slightly — even adding a word — to resubmit it. This is a known deferred edge, documented in the design doc's Error-handling section.
