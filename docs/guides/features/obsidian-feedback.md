...
- **Honest ✓** — `applied_at` is stamped only for the exact feedback rows that were in the prompt, and only when the LLM extraction parsed (`src/workflows/process-llm.ts`). If Ollama returned garbage and the note degraded to defaults, your feedback stays visibly pending (plain text, no ✓) — and it **is auto-retried**: at the end of every derivation pass, any still-unapplied feedback re-pends the note (`rependIfUnappliedFeedback` in `src/lib/vault-feedback.ts`), so the next process-llm cycle (every 5 min) re-derives with it. The same guard catches feedback that arrives mid-derivation. Fallback only if something is truly stuck (plain text, no ✓, across many cycles — e.g. Ollama persistently returning garbage): edit the text slightly — even adding a word — to force a fresh ingest.
...
- **Logs:** `logs/vault-feedback.log` (pino stdout — scan counts and `errorSamples`; also mirrored in `logs/selene.log`); `logs/vault-feedback.error.log` gets stderr only, i.e. hard crashes. (Dev paths; under `~/selene-prod/logs/` for the prod agent once installed.) The hourly export also logs its pre-export scan in `logs/export-obsidian.log`.
...
| No ✓ after ~1.5 hours | Check `logs/vault-feedback.log` (also mirrored in `logs/selene.log`) and run the scan manually (`npx ts-node src/workflows/vault-feedback.ts`) — is your note counted as `unmatched`? Then the file lacks `selene_id:` frontmatter or carries an id Selene doesn't recognize (hand-made files are skipped by design). Otherwise: is Ollama up (`curl http://localhost:11434/api/tags`)? Is there a pending backlog (`selene-inspect counts`)? If the extraction degraded (see below), the note re-pends automatically and retries on the next process-llm cycle — only if it stays plain across many cycles, edit the text slightly (even adding a word) to force a fresh ingest as a fallback. |
| Feedback shows ✓ but the filing didn't change much | The LLM may legitimately land on a similar filing. Your intent is permanent context now — it rides along on *every* future derivation (including rebuilds), so it still compounds. If extraction degraded (parse failure), the text stays plain (no ✓) rather than falsely showing ✓ — and the note re-pends automatically, so it retries on the next process-llm cycle. If it's truly stuck (plain with no ✓ across many cycles), edit the text slightly — even adding a word — to force a fresh ingest as a fallback. |
...
| Scan exits non-zero | Per-file errors; `logs/vault-feedback.log` (and `logs/selene.log`) carries `errorSamples` (filename + message, never note content). `logs/vault-feedback.error.log` only gets stderr — check it for hard crashes. |
