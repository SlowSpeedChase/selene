...
| 2026-05-30 | [2026-05-30-idempotent-obsidian-reexport-design.md](2026-05-30-idempotent-obsidian-reexport-design.md) | obsidian, export, constellation, bugfix | **Fixes the `exported_to_obsidian` write-once gate.** Notes export exactly once, so Knowledge Constellation Phase A's `parent:: [[cluster]]` edges reached only the ~40 notes exported after Phase A shipped — the ~254 older notes have no edges and never will (prod-log confirmed: one 40-note run, then all-zero runs). Fix: replace the boolean gate with a **rendered-output content hash** so the export is idempotent + self-healing (cluster/essence/theme change → rewrite; unchanged → skip). First run backfills `parent::` to the whole corpus. **Critical constraint:** hash the FULL rendered markdown (body + frontmatter + `parent::` Dataview block), not just the body, or it re-freezes the edges. **This IS the PKM Track 3 churn-guard work — build them together.** Root-caused 2026-05-30 session. ~half-day. |
