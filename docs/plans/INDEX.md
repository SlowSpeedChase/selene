...
|------|----------|--------|-------|
| 2026-06-13 | [2026-06-13-act0-daily-gift-design.md](2026-06-13-act0-daily-gift-design.md) | `feat/act0-daily-gift` | **Act 0 — The Daily Gift.** Grafts a "things I noticed for you" section onto the existing daily worksheet: ≤3 surfaced notes via slot roles (buried_treasure / connection / heating_up), four guilt-free reaction taps (important / keep / not now / let go), bundled with the worksheet submit. One new `attention_log` table in `facts.db` (precious); salience score = HN-style decay on engagement_count. New `gift_surface` WorksheetFieldKind; iPad `GiftSurfaceView` renders above the pencil fields. Cold-start: slots fall back to recency/random with no data. Establishes the daily habit + attention dataset Act 1 builds on. |
