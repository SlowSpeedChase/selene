...
| 2026-05-31 | [2026-05-31-fact-store-design.md](2026-05-31-fact-store-design.md) | architecture, data-integrity, regeneration, sqlite | **⏸ STATUS 2026-05-31: Ph1 IMPLEMENTED & e2e-validated on `feat/fact-store` (22 commits, 187 unit tests + full real-Ollama e2e green: fresh note capture→processed→clustered→exported, 0 SQLITE_BUSY/106k ops). NOT merged (effectively In Progress). GATE before merge = a prod-cutover runbook: merging auto-deploys, but the watcher won't run the migration → split-code on the un-migrated prod DB is incoherent; cutover must stop prod agents → run `scripts/migrate-to-fact-store.ts` on the real DB → deploy → verify → restart (or add auto-migrate-on-startup). The e2e caught + fixed a ship-blocker (`ALTER RENAME` repoints child FKs at the frozen backup → stripped `raw_notes` FK from `processed_notes`/`note_embeddings`). Orthogonal dev→prod vault-path bug still open.** — **Ingest-only fact store + regenerable derived DB.** Split the single DB into **two files**: `facts.db` (PRECIOUS — `captured_notes` append-only + human `category_overrides`/`review_state`, Time-Machine-backed) and `selene.db` (DISPOSABLE — all LLM/derived + pipeline bookkeeping, read-through `ATTACH`). "Separate by durability, not source": the file boundary *is* the provenance flag. Rebuild = wipe `selene.db` → re-derive from facts → re-apply human layer last (keyed on `source_uuid` + category **slug**, never churning surrogate ids — verified safe: `pkm_review_state` keys on note id, clusters key on slug). "Pending" becomes derivation-absence (the lone `db.ts:77` status query). **Re-derives, does not restore** (LLM non-deterministic). Phased: **Ph1 the split** (<1wk, Ready) → Ph2 `rebuild` cmd + validation → Ph3 override *feature* (net-new). Does NOT fix the orthogonal dev→prod vault-path bug. Plan: [2026-05-31-fact-store-plan.md](2026-05-31-fact-store-plan.md) (Ph1, 10 TDD tasks, `raw_notes`-view compat strategy). Branch `feat/fact-store`. |
| 2026-05-31 | [2026-05-31-fact-store-cutover-design.md](2026-05-31-fact-store-cutover-design.md) | ops, migration, prod, deploy | **The prod cutover runbook for the fact-store split** (the gate before merging Phase 1). **Hybrid:** (A) an `ensure-migrated` guard in `db.ts` startup — dev/clone **auto-migrates** (self-heal), prod **fails loud** if un-migrated (can never silently run the incoherent split); (B) `scripts/cutover-prod.sh`, one operator-run orchestrator: pre-flight build-gate → pause watcher → stop `com.selene.prod.*` → **verified backup** → `migrate-to-fact-store.ts` → **Gate 1** (content-free inspect + capture→pending probe) → `deploy-prod.sh` → restart → **Gate 2** (live /health + probe) → resume watcher, with **auto-rollback** to byte-for-byte single-file on either gate. Brief downtime window is inherent (old code can't read a migrated DB; new code can't safely read an un-migrated one). Claude writes + /tmp-tests it but **never runs it against prod** (prod-data guard) → every check content-free by construction. Branch `feat/fact-store`. |
