...
| Date | Document | Topic | Notes |
|------|----------|-------|-------|
| 2026-06-08 | [2026-06-08-freeform-diagram-loop-design.md](2026-06-08-freeform-diagram-loop-design.md) | ipad, apple-pencil, freeform, diagrams, visual-thinking | **Freeform ⇄ repo visual thinking loop.** An ongoing medium: user sketches "how Selene should work" on iPad in **Apple Freeform**; the only artifact that leaves Freeform (sealed app) is a share-sheet **PNG**, dropped into an **iCloud Drive inbox** (`Selene-Diagrams/` — *not* the guarded Obsidian vault). Claude **reads the PNG directly** (no OCR — `Read` renders images) and gives back design docs/plans + a committed keeper (`docs/diagrams/<topic>.png` + interpretation `.md`). **Seed = existing repo docs:** Claude renders the current system to a PNG to start from — anchored to the drift-proof generated `SYSTEM-MAP.md` (Option C), because the hand-drawn `backend-block-diagrams.md` was found **stale** (missing `folio-feedback`/`generate-worksheet`/`synthesize-topics`); one-time fix via `diagram-sync` + a standing "regenerate-before-seed" rule keep the seed always-current. One export tap per lap; no new daemon. User-facing → guide `diagram-loop.md` on wrap-up. |
