...
|------|----------|-------|-------|
| 2026-06-09 | [2026-06-09-macos27-apple-native-reevaluation-design.md](2026-06-09-macos27-apple-native-reevaluation-design.md) | strategy, foundation-models, private-cloud-compute, app-intents, lumen, product-direction | **Strategic umbrella: lean Apple-native off the WWDC 2026 (PCC + Foundation Models + Siri/App Intents) openings.** PCC is now free to developers (no keys, on-device→PCC→3rd-party auto-routing, `fm` CLI, Vision OCRTool, App Intents-only Siri) — dissolving the three constraints that forced Selene's split (Ollama needs a box / App Store can't ship inference / capture needs a webhook). **Priorities (user):** 1° *something I use* = already met by Selene → a **protect** goal; 2° *sellable* = Lumen, and the native features are what make a solo-built sellable app feasible. **Direction:** go native for **inference + OCR + Siri/App Intents interface**, with two guardrails — (1) *bet the engine, not the data* (notes stay SQLite + Obsidian markdown so the engine stays swappable), (2) *don't rip out unproven replacements* (embeddings stay on `nomic-embed`). **Compute:** on-device first (preserves "never leaves the Mac" + free + simple), PCC as optional escalation for the heavy "wants" synthesis only, decided by measurement; no manual token budgeting (auto-routed, both tiers free). **⚠️ Availability gap:** on-device FM + `fm` CLI ship with macOS 27, so an on-device engine swap is **Lumen-side / post-27-stable (~Sept)**, NOT a near-term change to the Selene TS pipeline on the stable mini (the table is Lumen's target arch). On-device "good enough" is an **assumption** (Apple on-device ~3B vs `mistral` 7B) the keystone measurement must confirm. **Path B chosen** (over A engine-swap-only / C flag-day collapse): Lumen becomes the sellable native home reading Selene as **oracle**; Selene stays the untouched daily driver; Lumen takes over only at **parity on all four** — capture friction · processing quality · surfacing back · history intact — by *earned replacement*, never a flag-day. **Consequence:** do **not** wipe the mini that runs prod Selene onto the beta (violates 1°) — native/beta work lives on a separate APFS volume / spare Mac. **Open Qs:** (1) ~~FM embedding API?~~ **RESOLVED** — FM has none (uses Spotlight RAG), but `NLContextualEmbedding` (NaturalLanguage fwk, 512-dim, on-device) is a native `nomic-embed` candidate, pending a clustering-parity check; Swift-only → Lumen-side; (2) does on-device match `mistral:7b` on the user's notes? (keystone measurement — on-device historically ~3B, a real risk). **Near-term:** macOS 27 on a separate volume → run the keystone measurement + `NLContextualEmbedding` clustering-parity check against the **dev showcase corpus** (`test_run='dev-seed'`, never prod notes); both inform **Lumen's** design. That spike is promotable to Ready first. Strategic umbrella over [context-blocks/apple-intelligence](2026-02-14-context-blocks-apple-intelligence-design.md) (tactical on-device slices) + [executive-assistant-wants](2026-06-07-executive-assistant-wants-design.md) (the PCC-candidate synthesis). |
| 2026-06-09 | [2026-06-09-macos27-migration-runbook.md](2026-06-09-macos27-migration-runbook.md) | runbook, migration, wipe, macos27, backup-restore | **Companion to the Path B strategy doc — the one-time *how-to* for wiping the mini to macOS 27 and getting back to developing.** Survives-the-wipe rule (runbook lives on GitHub branch + Extended drive, read from phone). Three-copy safety net (code→GitHub / notes+secrets→`/Volumes/Extended/selene-backup-2026-06-09` / vault→iCloud). Phases: 0 final pre-wipe (refresh backup, copy runbook + `~/.ollama` models to Extended), 1 wipe (**erase only `Macintosh HD` — never `Elements`/`Extended`**) + install beta, 2 toolchain (Xcode CLT, Homebrew `/opt/homebrew`, Node v22, gh/Tailscale auth, Ollama restore-or-repull), 3 restore Selene **dev-first** (clone repos, restore data, `npm install` recompiles `better-sqlite3` = #1 breakage risk, **keep prod offline** — don't reinstall prod launchd yet, pushing `main` IS a prod deploy), 4 verify (tsc+test, dev batch, vault, Lumen builds), 5 the **validation spike** (`fm` vs `mistral:7b` + `NLContextualEmbedding` parity on `test_run='dev-seed'`). Triage table for the common breakages. |
