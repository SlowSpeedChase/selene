...
| Date | Document | Topic | Notes |
|------|----------|-------|-------|
| 2026-05-31 | [2026-05-31-living-system-map-design.md](2026-05-31-living-system-map-design.md) | documentation, comprehension, drift, codegen, hooks | A codebase-comprehension system that can't go stale. **Root cause found:** system *facts* (which workflows exist, schedule, reads/writes) are hand-copied into prose in several places, so they drift — `CLAUDE.md` says **6** workflows, reality is **12**, the block-diagram doc says 9. Fix = a 3-level **zoom ladder** (CLAUDE.md pointers → generated `docs/SYSTEM-MAP.md` index → block-diagrams + code) where the L1 inventory is **generated** from `src/workflows/*.ts` + `launchd/*.plist` (between `<!-- GENERATED -->` markers; hand-written meaning lives outside them) and a Stop-hook `--check` makes drift impossible to ignore. Builds on existing `diagram-sync`/`launchd-auditor` machinery. YAGNI: no cross-repo map, no scheduled audit agent, no DB-schema gen yet. First win: regenerate to the true 12 + retire `CLAUDE.md`'s stale count. Plan: [2026-05-31-living-system-map-plan.md](2026-05-31-living-system-map-plan.md). |
