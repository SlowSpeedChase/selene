...
| Date | Document | Topic | Notes |
|------|----------|-------|-------|
| 2026-05-31 | [2026-05-31-fact-store-design.md](2026-05-31-fact-store-design.md) | architecture, data-integrity, regeneration, sqlite | **Ingest-only fact store + regenerable derived DB.** Split the single DB into **two files**: `facts.db` (PRECIOUS — `captured_notes` append-only + human `category_overrides`/`review_state`, Time-Machine-backed) and `selene.db` (DISPOSABLE — all LLM/derived + pipeline bookkeeping, read-through `ATTACH`). "Separate by durability, not source": the file boundary *is* the provenance flag. Rebuild = wipe `selene.db` → re-derive from facts → re-apply human layer last (keyed on `source_uuid` + category **slug**, never churning surrogate ids — verified safe: `pkm_review_state` keys on note id, clusters key on slug). "Pending" becomes derivation-absence (the lone `db.ts:77` status query). **Re-derives, does not restore** (LLM non-deterministic). Phased: **Ph1 the split** (<1wk, Ready) → Ph2 `rebuild` cmd + validation → Ph3 override *feature* (net-new). Does NOT fix the orthogonal dev→prod vault-path bug. |
