...
| Date | Document | Topic | Notes |
|------|----------|-------|-------|
| 2026-06-01 | [2026-06-01-fact-store-rebuild-design.md](2026-06-01-fact-store-rebuild-design.md) | architecture, regeneration, sqlite, ops | **Fact-store Phase 2 — the `rebuild` command.** Now that Ph1's two-file split is LIVE in prod, this is the command that makes `selene.db` actually disposable: wipe it, re-derive the whole corpus from `facts.db` (process-llm → distill → synthesize → export), validate, keep-or-rollback. Serves all three drivers equally (reprocess-on-upgrade / disaster recovery / safe experimentation). **Approach B (chosen):** typed, jest-tested TS core (`scripts/rebuild.ts` + pure `src/lib/rebuild-core.ts`) owns all logic; bash confined to `launchctl` via a NEW shared `scripts/lib/prod-agents.sh` **extracted** from `cutover-prod.sh` (regression net = `verify-cutover.sh` stays 45/45). **Dev-first, prod-capable** (Claude builds/tests on /tmp + dev `.backup`, never runs vs prod; operator runs the prod path). **Whole-corpus only**, **through Obsidian export**. Safety: verified backup of `selene.db` (facts.db never touched — precious/TM-backed), **auto-rollback** on a failed gate. Gate = pure `verdict(PRE,POST,thresholds)`: **≥95% coverage floor** + **≤20% downward drift** per metric, both env-overridable, zero-baseline drift skipped. Crash-between-wipe-and-validate is self-healing (pending remainder finished by the live pipeline). `--dry-run` + `SIMULATE_*` hooks prove both rollback paths; `verify-rebuild.sh` is the e2e harness. Human layer (`review_state`) survives automatically (lives in facts.db) — explicit re-apply + `category_overrides` are **Phase 3**. Guide → `releases.md`. Branch `feat/fact-store`. |
