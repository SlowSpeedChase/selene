...
|------|----------|-------|-------|
| 2026-06-09 | [2026-06-09-macos27-apple-native-reevaluation-design.md](2026-06-09-macos27-apple-native-reevaluation-design.md) | strategy, foundation-models, private-cloud-compute, app-intents, lumen, product-direction | **Strategic umbrella: lean Apple-native off WWDC 2026 (PCC + Foundation Models + Siri/App Intents).** PCC free to developers (auto-routing, `fm` CLI, Vision OCRTool, App Intents-only Siri). Direction: native for inference + OCR + Siri/App Intents interface, with two guardrails — bet the engine not the data (SQLite + Obsidian markdown stays swappable), don't rip out unproven replacements. **Path B chosen:** Lumen becomes the sellable native home reading Selene as oracle; Selene stays the untouched daily driver; Lumen takes over by earned replacement. On-device FM + `fm` CLI ship with macOS 27 — an on-device engine swap is Lumen-side / post-27-stable (~Sept). Runbook: [2026-06-09-macos27-migration-runbook.md](2026-06-09-macos27-migration-runbook.md). |
| 2026-06-09 | [2026-06-09-macos27-migration-runbook.md](2026-06-09-macos27-migration-runbook.md) | runbook, migration, wipe, macos27, backup-restore | **Companion to the Path B strategy — one-time how-to for wiping the mini to macOS 27 and getting back to developing.** Three-copy safety net (code→GitHub / notes+secrets→Extended drive / vault→iCloud). Phases: 0 final pre-wipe → 1 wipe + install beta → 2 toolchain → 3 restore Selene dev-first → 4 verify → 5 the validation spike (`fm` vs `mistral:7b` + `NLContextualEmbedding` parity on `test_run='dev-seed'`). |
...
| 2026-02-14 | [2026-02-14-context-blocks-apple-intelligence-design.md](2026-02-14-context-blocks-apple-intelligence-design.md) | apple-intelligence, foundation-models, ios27, on-device, swift | **REVISED 2026-06-09 (was wrongly filed under Done — never shipped).** Original "Context Blocks" scope (chunk-retrieval thread-chat in **SeleneChat**) is void: SeleneChat + the thread system were **archived in the 2026-03-21 simplification**, and its iOS 26 Foundation Models assumptions (fixed ~3B / 4K context) were **superseded by iOS/macOS 27** (larger model, expanded context, `@Generable` structured output, multi-step tool calling, multimodal input, on-device fine-tuning, `SpeechAnalyzer`, App Intents-only Siri, Python/Linux SDK). Re-scoped into independent on-device slices ranked by leverage — **A: `@Generable` structured extraction (recommended first)**, B: Whisper→`SpeechAnalyzer` transcription, C: on-device multimodal OCR (replace Claude Vision / `qwen2.5vl`), D: Siri/App Intents capture+query, E: provider-abstraction. Integration lands at the **Apple edge** (shipped **SeleneMarkup** iPad app / a Swift helper), not the Node core — unless an FM Python/Linux-SDK spike proves server-side use. The 10-task Swift plan (`...-plan.md`) is **obsolete/superseded**. Needs a slice pick + Swift-edge-vs-Node-core decision + acceptance criteria before it can move to Ready. |
...
| Date | Document | Topic | Notes |
|------|----------|-------|-------|
| 2026-06-10 | [2026-06-10-obsidian-feedback-loop-design.md](2026-06-10-obsidian-feedback-loop-design.md) | feedback, obsidian, classification, facts-db | **Obsidian feedback loop — "Your note."** Free-text intent feedback typed directly in the exported note (a `## ✍️ Your note` section, works in Obsidian Mac/iPad mid-ExcaliBrain-browse) that re-files the note and teaches the classifier. New `vault-feedback` workflow (15-min launchd scan) → precious `note_feedback` table in `facts.db` (keyed `source_uuid`, snapshots `original_filing`) → **re-file = delete the derived rows** (pending-as-derivation-absence; process-llm re-derives with the intent injected into EXTRACT_PROMPT/essence) → exporter renders the feedback back as an applied-✓ block (preserve-on-render guard means no export ordering can clobber un-ingested text; `selene_uuid` added to frontmatter for file→note identity). Feedback survives `rebuild` by construction (facts-side, re-applied as prompt context on every re-derivation). **Delivers the spirit of fact-store Ph3** (`category_overrides` demoted to "the hard pin, if re-derivation proves disobedient"). **Ph2 (1–2 days, gated on Ph1 data):** last ~5 corrections as few-shot examples in the classification prompt — filed-by-function, not surface topic, for future notes. Motivating example: the sentence-diagramming note (filed `Academic Achievement`; meant "a skill I possess and enjoy — remember it"). Ph1 < 1 week. Plan: [2026-06-10-obsidian-feedback-loop-plan.md](2026-06-10-obsidian-feedback-loop-plan.md) (11 TDD tasks; keys on `captured_notes.id`/`selene_id` frontmatter — NOT source_uuid, older notes have it NULL; re-pend via `setNoteState` status flip, not row deletes). Branch `feat/obsidian-feedback-loop`. |
...
|------|----------|--------|-------|
| 2026-06-08 | [2026-06-08-freeform-diagram-loop-design.md](2026-06-08-freeform-diagram-loop-design.md) | `feat/freeform-diagram-loop` | **Freeform ⇄ repo visual thinking loop — mechanism BUILT, awaiting first-lap validation.** Mac side proven end-to-end: `scripts/render-seed-diagram.ts` (tested core `src/lib/seed-diagram.ts`, stale-`SYSTEM-MAP` guard) renders the current system to a PNG in the iCloud inbox `Selene-Diagrams/`; Claude `Read` it cleanly. `backend-block-diagrams.md` un-stale fix shipped (added folio-feedback/generate-worksheet/synthesize-topics). Guide: `docs/guides/features/diagram-loop.md`. **NOT yet validated:** (1) does the seed PNG actually surface on the iPad (iCloud Drive → Files → Freeform — upload lag / `.icloud` placeholder risk); (2) can Claude read real **handwriting/Pencil marks**, not just the printed seed — the headline capability, still unproven. Merge + mark Done only after one real lap. Plan: [2026-06-08-freeform-diagram-loop-plan.md](2026-06-08-freeform-diagram-loop-plan.md). |
...
|------|----------|-----------|-------|
| 2026-06-13 | [2026-06-13-act0-daily-gift-design.md](2026-06-13-act0-daily-gift-design.md) | 2026-06-14 | **Act 0 — The Daily Gift. Merged to main + deployed.** "Things I noticed for you" section grafted onto the existing daily worksheet: ≤3 surfaced notes via 3 slot roles (buried_treasure / connection / heating_up), four guilt-free reaction taps (important / keep / not now / let go), bundled with worksheet submit. `attention_log` table in `facts.db` (precious — taps survive `rebuild`); salience score = HN-style decay on engagement_count. `gift_surface` WorksheetFieldKind; iPad `GiftSurfaceView` renders above pencil fields. Cold-start fallback: recency/random with no data. Establishes the daily habit + attention dataset Act 1 builds on. 390 TS tests + 27 Swift tests passing, tsc clean. Guide: `docs/guides/features/daily-gift.md`. |
| 2026-06-10 | [2026-06-10-obsidian-feedback-loop-design.md](2026-06-10-obsidian-feedback-loop-design.md) | 2026-06-11 | **Obsidian feedback loop — "Your note" (Ph1). Merged PR #58 2026-06-11; deployed (vault-feedback agent running).** Free-text intent typed under `## ✍️ Your note` in any exported note re-files the note and teaches the classifier. `vault-feedback` workflow (15-min launchd scan; export scans pre-render) → precious `note_feedback` table in `facts.db` (keyed `captured_notes.id` via `selene_id` frontmatter) → re-pend via `setNoteState` → process-llm/distill re-derive with intent in-prompt → exporter renders applied-✓ block. Survives `rebuild` by construction; delivers spirit of fact-store Ph3. **Ph2 (gated on Ph1 data):** few-shot corrections in classification prompt remains. Plan: [2026-06-10-obsidian-feedback-loop-plan.md](2026-06-10-obsidian-feedback-loop-plan.md). Guide: `docs/guides/features/obsidian-feedback.md`. |
| 2026-06-06 | [2026-06-06-maintainability-cleanup.md](2026-06-06-maintainability-cleanup.md) | 2026-06-09 | **Code/glue maintainability cleanup. Merged PR #56 2026-06-06; deployed 2026-06-09.** ~1,453 lines of low-risk removal across 12 ranked tasks: dead subsystems from 2026-03-21 simplification (db.ts thread/chat/memory, ContextBuilder, self-referential orphans, bit-rotted scripts + dead package.json entries), two drift fixes (.env.example, launchd install list silently skipping folio-feedback), six low-risk consolidations. 8-finder review + skeptical verification; 4 over-abstraction candidates explicitly rejected. Executed via subagent-driven development. |
| 2026-05-29 | [2026-05-29-knowledge-constellation-design.md](2026-05-29-knowledge-constellation-design.md) | 2026-06-11 | **Phase A + Phase B both shipped.** Phase A (2026-05-30): `export-obsidian.ts` emits `parent:: [[cluster]]` + `Constellation/<cluster>.md` index notes; auto-populates iCloud vault hourly. Phase B (2026-06-11): `friend:: [[basename]]` top-5 cosine-similarity edges — bidirectional lateral flight in ExcaliBrain. `note_connections` backfilled (6,794 edges / 336 notes). See separate Phase B design entry. |
| 2026-06-11 | [2026-06-11-constellation-phase-b-design.md](2026-06-11-constellation-phase-b-design.md) | 2026-06-11 | **Constellation Phase B — note↔note `friend::` edges.** Adds `friend:: [[basename]]` Dataview fields (top-5 by cosine similarity) to every exported note so ExcaliBrain renders lateral note↔note flight alongside the cluster→note hierarchy from Phase A. New `loadNoteFriends` + `buildFriendFields` in `constellation.ts`; wired into `obsidian-render.ts` via a new `friendBasenames` param on `renderNoteMarkdown`. Bidirectional query (UNION source→target + target→source); top-N cap = 5. Precondition satisfied: `note_connections` backfilled 2026-06-11 (6,794 edges / 336 notes). Notes with no connections above the 0.75 threshold export cleanly with no `friend::` block. Plan: [2026-06-11-constellation-phase-b-plan.md](2026-06-11-constellation-phase-b-plan.md) (5 TDD tasks). Guide: `docs/guides/features/knowledge-constellation.md`. |
