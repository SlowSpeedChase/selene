...
|------|----------|--------|-------|
| 2026-06-10 | [2026-06-10-obsidian-feedback-loop-design.md](2026-06-10-obsidian-feedback-loop-design.md) | `feat/obsidian-feedback-loop` | **Obsidian feedback loop — "Your note" (Ph1).** Free-text intent typed directly under the `## ✍️ Your note` section of any exported note (Obsidian Mac/iPad, mid-ExcaliBrain-browse) re-files the note and teaches the classifier. `vault-feedback` workflow (15-min launchd scan; export also scans pre-render) → precious `note_feedback` table in `facts.db` (keyed `captured_notes.id` via `selene_id` frontmatter, snapshots `original_filing`) → re-pend via `setNoteState` status flip → process-llm/distill re-derive with the intent in-prompt → exporter renders an applied-✓ block (preserve-on-render guard; `applied_at` stamped only on parsed extraction). Survives `rebuild` by construction; delivers the spirit of fact-store Ph3. **Ph2 (gated on Ph1 data):** last ~5 corrections as few-shot examples in the classification prompt. Plan: [2026-06-10-obsidian-feedback-loop-plan.md](2026-06-10-obsidian-feedback-loop-plan.md). Guide: `docs/guides/features/obsidian-feedback.md`. **Implementation complete on branch, e2e verified 2026-06-11; awaiting review/merge.** Post-merge operator step: `./scripts/install-prod.sh` (new agent). |
