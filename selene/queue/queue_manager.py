...
class QueueManager:
    ...
    def __init__(
        self, processing_queue: Optional[ProcessingQueue] = None, max_workers: int = 3
    ):
        """
        ...
        try:
            ...
            if not processor:
                await self.processing_queue.fail_item(
                    item.id, f"Failed to get processor: {item.processor_type}"
                )
                return False
            ...
            if item.file_path:
                ...
                if not file_path.exists():
                    await self.processing_queue.fail_item(
                        item.id, f"File not found: {item.file_path}"
                    )
                    return False

                result = await processor.process_file(
                    file_path, task=item.task, **item.metadata
                )
            elif item.content:
                # Process content directly
                result = await processor.process(
                    item.content, task=item.task, **item.metadata
                )
            else:
                await self.processing_queue.fail_item(
                    item.id, "No file path or content provided"
                )
                return False
            ...
            if result.success:
                ...
                if item.metadata.get("store_in_vector_db", False):
                    await self._store_in_vector_db(
                        item, result.content, result_metadata
                    )

                # Mark as completed
                await self.processing_queue.complete_item(
                    item.id, result.content, result_metadata
                )

                logger.info(
                    f"Successfully processed {item.task} for: {item.file_path or 'content'}"
                )
                return True
            else:
                await self.processing_queue.fail_item(
                    item.id, result.error or "Processing failed with unknown error"
                )
                return False

        except Exception as e:
            await self.processing_queue.fail_item(
                item.id, f"Processing exception: {str(e)}"
            )
            logger.error(f"Error processing item {item.id}: {e}")
            return False

    async def _store_in_vector_db(
        self, item: QueueItem, processed_content: str, result_metadata: Dict[str, Any]
    ):
        ...
        try:
            ...
            if not vector_processor:
                logger.warning(
                    "Vector processor not available, skipping vector storage"
                )
                return
            ...
            result_metadata["vector_storage"] = {"success": False, "error": str(e)}
    ...
    async def _process_vector_item(self, item: QueueItem) -> bool:
        ...
        try:
            ...
            if not vector_processor:
                await self.processing_queue.fail_item(
                    item.id, "Vector processor not available"
                )
                return False
            ...
            if item.content:
                ...
                result = await vector_processor.process(
                    "", task=item.task, **item.metadata
                )

            if result.success:
                await self.processing_queue.complete_item(
                    item.id, result.content, result.metadata
                )
                return True
            else:
                await self.processing_queue.fail_item(
                    item.id, result.error or "Vector operation failed"
                )
                return False

        except Exception as e:
            await self.processing_queue.fail_item(
                item.id, f"Vector processing exception: {str(e)}"
            )
            return False
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if item.item_type == QueueItemType.FILE_PROCESS:
                    ...
                    logger.warning(
                        f"Batch processing not yet implemented for item {item.id}"
                    )
                    await self.processing_queue.fail_item(
                        item.id, "Batch processing not implemented"
                    )
                else:
                    logger.warning(f"Unknown item type: {item.item_type}")
                    await self.processing_queue.fail_item(
                        item.id, f"Unknown item type: {item.item_type}"
                    )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "queue": queue_status,
            "statistics": {**self._worker_stats, "uptime_seconds": uptime},
            "processors": {
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_items": {
                "pending": [
                    item.to_dict()
                    for item in self.processing_queue.get_items_by_status(
