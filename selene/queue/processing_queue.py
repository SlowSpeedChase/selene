...
class QueueItem:
    ...
    def mark_completed(self, result_content: str = "", result_metadata: Dict[str, Any] = None):
    ...
    def can_retry(self) -> bool:
        ...
        return (self.status == QueueItemStatus.FAILED and 
...
class ProcessingQueue:
    ...
    async def complete_item(self, item_id: str, result_content: str = "", result_metadata: Dict[str, Any] = None):
    ...
    async def clear_completed(self, max_age_hours: float = 24.0):
        ...
        async with self._queue_lock:
            ...
            for item_id, item in self._completed.items():
                if item.completed_at and (current_time - item.completed_at) > max_age_seconds:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        total_items = (self._stats["total_processed"] + 
                      self._stats["total_failed"] + 
                      self._stats["total_cancelled"])
        
        avg_processing_time = 0.0
        if self._stats["total_processed"] > 0:
            avg_processing_time = self._stats["processing_time_total"] / self._stats["total_processed"]
