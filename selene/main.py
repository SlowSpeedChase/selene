...
from selene.queue import QueueManager, ProcessingQueue
...
@app.command()
def process(
    content: Optional[str] = typer.Option(None, "--content", "-c", help="Content to process directly"),
    file_path: Optional[Path] = typer.Option(None, "--file", "-f", help="File to process"),
    task: str = typer.Option("enhance", "--task", "-t", help="Processing task: summarize, enhance, extract_insights, questions, classify"),
    processor: str = typer.Option("ollama", "--processor", "-p", help="Processor type: ollama (local), openai (cloud), vector (local database)"),
    model: str = typer.Option("llama3.2", "--model", "-m", help="Model to use (llama3.2, mistral, gpt-4o-mini, etc.)"),
    output: Optional[Path] = typer.Option(None, "--output", "-o", help="Output file path"),
    api_key: Optional[str] = typer.Option(None, "--api-key", help="OpenAI API key (only for openai processor)"),
    ...
    if processor not in ["ollama", "openai", "vector"]:
        console.print(f"[red]Error: Invalid processor '{processor}'. Choose 'ollama', 'openai', or 'vector'.[/red]")
        raise typer.Exit(1)
    ...
    if processor == "openai":
        ...
        if not openai_key:
            console.print("[red]Error: OpenAI API key required for OpenAI processor. Use --api-key or set OPENAI_API_KEY environment variable.[/red]")
            raise typer.Exit(1)
    ...
    if content and file_path:
        console.print("[red]Error: Cannot use both --content and --file simultaneously[/red]")
        raise typer.Exit(1)
    ...
    async def run_processing():
        console.print(f"[cyan]Processing with {processor} processor, task: {task}[/cyan]")
        ...
        if file_path:
            result = await processor_instance.process_file(file_path, task=task, model=actual_model)
        else:
            result = await processor_instance.process(content, task=task, model=actual_model)
    ...
    # Display results
    if result.success:
        console.print(f"[green]✅ Processing completed in {result.processing_time:.2f}s[/green]")
...
@app.command()
def vector(
    action: str = typer.Argument(..., help="Vector action: store, search, retrieve, delete, list, stats"),
    content: Optional[str] = typer.Option(None, "--content", "-c", help="Content to process"),
    file_path: Optional[Path] = typer.Option(None, "--file", "-f", help="File to process"),
    doc_id: Optional[str] = typer.Option(None, "--id", help="Document ID for retrieve/delete operations"),
    query: Optional[str] = typer.Option(None, "--query", "-q", help="Search query"),
    n_results: int = typer.Option(5, "--results", "-n", help="Number of search results"),
    metadata: Optional[str] = typer.Option(None, "--metadata", "-m", help="JSON metadata for document"),
    db_path: str = typer.Option("./chroma_db", "--db-path", help="Vector database path"),
    collection: str = typer.Option("selene_notes", "--collection", help="Collection name"),
) -> None:
    ...
    if action not in valid_actions:
        console.print(f"[red]Error: Invalid action '{action}'. Choose from: {', '.join(valid_actions)}[/red]")
        raise typer.Exit(1)
    
    # Validate input based on action
    if action == "store":
        if not content and not file_path:
            console.print("[red]Error: Must provide either --content or --file for store action[/red]")
            raise typer.Exit(1)
    elif action == "search":
        if not query:
    ...
    # Display results
    if result.success:
        console.print(f"[green]✅ {action.title()} completed in {result.processing_time:.2f}s[/green]")
                ...
                if key == "results" and isinstance(value, list):
                    ...
                    for res in value:
                        search_table.add_row(
                            ...
                            res['document_id'][:20] + "..." if len(res['document_id']) > 20 else res['document_id'],
                            res['content_preview'][:50] + "..." if len(res['content_preview']) > 50 else res['content_preview']
                        )
                    ...
                    for doc in value:
                        ...
                        doc_table.add_row(
                            doc['document_id'][:20] + "..." if len(doc['document_id']) > 20 else doc['document_id'],
...
def doctor() -> None:
    ...
    try:
        ...
        if connection_info.get("connected"):
            console.print(f"   ✅ Ollama service running at {connection_info['base_url']}", style="green")
            ...
            console.print(f"   ❌ Cannot connect to Ollama at {ollama_processor.base_url}", style="red")
    ...
    try:
        ...
        if connection_info.get("connected"):
            ...
            if available_models:
                # Find best model
                best_model = ollama_processor._find_best_available_model(available_models)
                console.print(f"   🚀 Use this command: selene process --model {best_model} --content 'test'")
                
                # Suggest additional models
                suggested_models = ["llama3.2", "mistral", "phi3:mini"]
                missing_models = [m for m in suggested_models if m not in available_models]
...
def jira_setup() -> None:
    ...
    console.print("Your JIRA instance: [cyan]https://slowspeedchase.atlassian.net[/cyan]")
    console.print()
    ...
    async def check_jira_status():
        try:
            ...
            if not auth_success:
                ...
                console.print("💡 Run: [cyan]selene jira-setup[/cyan] for configuration help")
                return
            ...
            if tickets:
                ...
                console.print("💡 Run: [cyan]selene jira sync[/cyan] to create project tickets")
...
def jira_sync() -> None:
    ...
    async def sync_jira_project():
        try:
            ...
            if created_tickets:
                console.print(f"[green]✅ Processed {len(created_tickets)} tickets[/green]")
            ...
            if sync_results.get("error"):
                ...
                if sync_results.get("errors"):
                    console.print(f"[yellow]⚠️  Errors: {len(sync_results['errors'])}[/yellow]")
...
@app.command()
def monitor(
    action: str = typer.Argument(..., help="Monitor action: start, stop, status, add, remove, config"),
    path: Optional[Path] = typer.Option(None, "--path", "-p", help="Directory path for add/remove actions"),
    patterns: Optional[str] = typer.Option(None, "--patterns", help="File patterns (comma-separated, e.g., '*.txt,*.md')"),
    recursive: bool = typer.Option(True, "--recursive/--no-recursive", help="Watch subdirectories"),
    auto_process: bool = typer.Option(True, "--auto-process/--no-auto-process", help="Enable automatic processing"),
    tasks: Optional[str] = typer.Option("summarize,extract_insights", "--tasks", help="Processing tasks (comma-separated)"),
    processor: str = typer.Option("ollama", "--processor", help="Default processor (ollama, openai, vector)"),
    config_file: str = typer.Option(".monitor-config.yaml", "--config", help="Configuration file path"),
) -> None:
    """Manage file monitoring and auto-processing system."""
    setup_logging()
    
    # Validate action
    valid_actions = ["start", "stop", "status", "add", "remove", "config", "process-existing"]
    if action not in valid_actions:
        console.print(f"[red]Error: Invalid action '{action}'. Choose from: {', '.join(valid_actions)}[/red]")
        raise typer.Exit(1)
    ...
    async def run_monitor_action():
        try:
            if action == "config":
                ...
                table.add_row("Max Concurrent Jobs", str(summary["max_concurrent_jobs"]))
                table.add_row("Watched Directories", str(summary["watched_directories_count"]))
                table.add_row("Supported Extensions", ", ".join(config.supported_extensions[:5]) + "...")
                ...
                if config.watched_directories:
                    ...
                    console.print("[yellow]No directories configured for monitoring[/yellow]")
                ...
                pattern_list = patterns.split(",") if patterns else ["*.txt", "*.md", "*.pdf", "*.docx"]
                task_list = tasks.split(",") if tasks else ["summarize", "extract_insights"]
                ...
                if success:
                    # Save config
                    config.save_to_file(config_file)
                    console.print(f"[green]✅ Removed watched directory: {path}[/green]")
                else:
                    console.print(f"[yellow]⚠️  Directory not found in watch list: {path}[/yellow]")
            ...
            if config_issues:
                console.print(f"[red]Configuration issues: {', '.join(config_issues)}[/red]")
                console.print("💡 Run: [cyan]selene monitor config[/cyan] to check configuration")
                raise typer.Exit(1)
            ...
            queue_manager = QueueManager(processing_queue, max_workers=config.max_concurrent_jobs)
            file_watcher = FileWatcher(config, processing_queue)
            
            if action == "start":
                console.print("[bold blue]🚀 Starting File Monitoring System[/bold blue]")
                console.print("=" * 50)
                console.print()
                ...
                console.print("[green]✅ File monitoring system started successfully![/green]")
                console.print()
                
                # Show status
                status = file_watcher.get_status_summary()
                
                console.print(f"📁 Watching {len(status['watcher_status']['watched_paths'])} directories:")
                ...
                console.print("[yellow]⚠️  Status check for detached monitoring not yet implemented[/yellow]")
                console.print("💡 Current configuration:")
                
                summary = config.get_summary()
                console.print(f"   📁 Watched directories: {summary['watched_directories_count']}")
                console.print(f"   ⚙️  Default processor: {summary['default_processor']}")
                console.print(f"   🔄 Processing enabled: {summary['processing_enabled']}")
                ...
                while True:
                    ...
                    console.print(f"   Queue: {queue_size}, Processing: {processing_count}")
                ...
                console.print("[yellow]⚠️  Stop command for detached monitoring not yet implemented[/yellow]")
