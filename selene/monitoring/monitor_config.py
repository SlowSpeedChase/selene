...
class WatchedDirectory:
    ...
    path: str
    patterns: List[str] = field(default_factory=lambda: ["*.txt", "*.md", "*.pdf", "*.docx"])
    recursive: bool = True
    auto_process: bool = True
    processing_tasks: List[str] = field(default_factory=lambda: ["summarize", "extract_insights"])
        ...
        try:
            ...
            for dir_data in data.get('watched_directories', []):
                watched_dir = WatchedDirectory(
                    path=dir_data['path'],
                    patterns=dir_data.get('patterns', ["*.txt", "*.md", "*.pdf", "*.docx"]),
                    recursive=dir_data.get('recursive', True),
                    auto_process=dir_data.get('auto_process', True),
                    processing_tasks=dir_data.get('processing_tasks', ["summarize", "extract_insights"]),
