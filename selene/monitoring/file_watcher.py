...
class FileEventHandler(FileSystemEventHandler):
    ...
    def _should_process_file(self, file_path: str, watched_dir: WatchedDirectory) -> bool:
    ...
    def on_moved(self, event: FileSystemEvent):
        ...
        logger.info(f"File moved: {getattr(event, 'src_path', 'unknown')} -> {event.dest_path}")
        self._schedule_callback(file_event)
...
class FileWatcher:
        ...
        try:
            ...
            for task in watched_dir.processing_tasks:
                ...
                if success:
                    ...
                    logger.warning(f"Failed to queue {task} task for: {event.file_path}")
    ...
    async def start_watching(self) -> bool:
        ...
        try:
            ...
            for watched_dir in self.config.watched_directories:
                ...
                logger.info(f"Watching directory: {path} (recursive: {watched_dir.recursive})")
            ...
            logger.info(f"File watcher started, monitoring {len(self._watched_paths)} directories")
            return True
    ...
    async def add_watched_directory(self, 
                                   path: str,
    ...
    async def process_existing_files(self, directory_path: Optional[str] = None):
        ...
        for watched_dir in directories_to_process:
            try:
                ...
                for pattern in watched_dir.patterns:
                    ...
                    for file_path in files:
                        if file_path.is_file():
                            ...
                            if (self.config.is_file_supported(str(file_path)) and 
                                not self.config.should_ignore_file(str(file_path))):
                ...
                logger.info(f"Processed {processed_files}/{total_files} existing files in {watched_dir.path}")
            
            except Exception as e:
                logger.error(f"Error processing existing files in {watched_dir.path}: {e}")
        
        logger.info(f"Batch processing complete: {processed_files} files queued from {total_files} total files")
