...
class EmbeddingService:
    ...
    async def _get_openai_embeddings(self, texts: List[str]) -> EmbeddingResult:
        ...
        try:
            ...
            return EmbeddingResult(
                ...
                processing_time=time.time() - start_time,
                metadata={"provider": "openai", "local": False, "usage": response.usage.total_tokens}
            )
    ...
    async def generate_embeddings(self, texts: List[str]) -> EmbeddingResult:
        ...
        if self.prefer_local:
            ...
            if result.success:
                logger.info(f"Local embeddings generated successfully with {result.model_used}")
                return result
            else:
        ...
        if result.success:
            logger.info(f"OpenAI embeddings generated successfully with {result.model_used}")
            return result
        else:
