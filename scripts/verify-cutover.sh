...
#   3. ROLLBACK PATH   — fresh snapshot, SIMULATE_GATE1_FAIL=1 → rollback_all runs: /tmp copy is
#                        byte-for-byte single-file again, rollback-prod.sh line PRINTED, "ROLLED BACK".
#
...
# rollback-prod.sh line PRINTED (code rollback stubbed).
echo "$RB_OUT" | grep -q "\[dry-run\] .*rollback-prod.sh" \
  && note_pass "code rollback was STUBBED ([dry-run] rollback-prod.sh ...)" \
  || note_fail "rollback-prod.sh dry-run line missing"
# "ROLLED BACK" notify printed (stubbed).
echo "$RB_OUT" | grep -q "ROLLED BACK" && note_pass "'ROLLED BACK' printed" || note_fail "'ROLLED BACK' message missing"
...
echo "  Rollback path:  $( echo "$RB_OUT" | grep -q 'ROLLED BACK' && echo 'restored single-file + ROLLED BACK' || echo FAIL )"
