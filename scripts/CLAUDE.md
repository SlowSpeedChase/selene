...
- **seed-dev-data.ts** - Insert ~500 fictional notes into the dev DB (pending status)
...
Generates ~500 PURELY FICTIONAL notes (zero real PII) spread over ~3 months and
emits them as a JSON array of `{title, content, created_at}`. Deterministically
seeded, so re-runs produce the same fixture. Invented ADHD knowledge-worker
themes: project ideas, reflections, reading notes, task thoughts, meeting notes.
...
Runs `generate-dev-fixture.py` (or reads its output via `--fixture`) and inserts
the notes into the dev database's `raw_notes` table in `'pending'` status
(`capture_type='dev-fixture'`, `test_run='dev-seed'`) so `dev-process-batch.sh`
can process them. Does NOT run the LLM itself.

**Safety:** refuses to run unless the target DB is explicitly marked
`_selene_metadata.environment='development'` — it re-checks the marker directly
rather than trusting `SELENE_ENV`, so it can never write to production.
